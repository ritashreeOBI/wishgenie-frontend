...
console.log(selectedVaraintDetails)
  ...
  const OrderHandler = async () => {
    try {
   setLoading(true)
   const {name , price , product_id , image , size, color_code } = selectedVaraintDetails
    const productDetail = {
        title: name,
        description: '',
        images: JSON.parse(sessionStorage.getItem('files')),
        sellingPrice:price,
        thumbnails: image,
        category: '',
        VariantId: product_id,
        quantity: 1,
        customerId: localStorage.getItem('u_id'),
        productSize:size,
        colorCode:color_code,
      }
      console.log(productDetail)

      const { data } = await axios.post(ADD_TO_CART, productDetail)
      console.log(data)
