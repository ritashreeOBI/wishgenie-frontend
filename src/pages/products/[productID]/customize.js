...
// import ImageEditor from '@/components/shared-components/editor/ImageEditor';
import Editor from '@/components/Editor';
import axios from 'axios';
import { MOCKUP_TEMPLATE, printFullApi } from '@/api/Api';
import { TestEditor } from '@/components/shared-components/editor/TestEditor';
// const CustomEditor = dynamic(() => import('@/components/shared-components/editor/CustomEditor'), {
//   ssr: false,
// });
   ...
   const [selectedTemplate , setSelectedTemplate] = useState({})
      ...
      const getMockup = async () => {
        ...
        .then(response => {
            ...
            // const filteredData = response?.data?.result?.templates
            // .filter(item => item.background_color !== null)
            // .reduce((uniqueData, item) => {
            //     if (!uniqueData.has(item.background_color)) {
            //         uniqueData.set(item.background_color, item);
            //     }
            //     return uniqueData;
            // }, new Map());

            // function filterElements(originalArray, skipCount) {
            //   const filteredArray = [];
            
            //   for (let index = 0; index < originalArray.length; index++) {
            //     if ((index + 1) % skipCount !== 0) {
           ...
           console.log()
            //const filteredArray = filterElements(response?.data?.result?.template, response?.data?.result?.variant_mapping[0].templates.);
              ...
              <div className=' flex flex-row  justify-between gap-2'>
                ...
                <div className='pb-4 pr-2 rounded-md grow gap-4 flex flex-col items-center mt-2 justify-between pt-4'>
                   ...
                   {/* <Editor path={productDetail?.image} />   */}
                    ...
                    <ImageEditor/> */}
                    {product?.image &&
                     <div  style={{width:750 ,  borderRadius:20 ,overflow:'hidden',
